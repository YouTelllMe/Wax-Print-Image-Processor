...
%Example:
%
%   p=FastPeakFind(imgThresh,6);
%   imagesc(imgThresh); hold on
...
%% defaults
if (nargin < 1)
    d=uint16(conv2(reshape(single( 2^14*(rand(1,1024*1024)>0.99995) ),[1024 1024]) ,fspecial('gaussian', 2015,5),'same')+2^8*rand(1024));
    imagesc(d);
end
...
if (nargin < 3)
    filt = (fspecial('gaussian', 15,5)); %if needed modify the filter according to the expected peaks sizes
end
if (nargin < 4)
    edg =2;
end
